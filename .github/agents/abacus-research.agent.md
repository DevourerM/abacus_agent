...
- 只在版本兼容性或错误诊断确实需要时识别 ABACUS 版本；优先读取已有日志，避免无必要地加载 module 或运行版本命令。
...
7. 先运行与当前问题直接相关的最小检查；展示即将执行的 `bsub`、`bkill` 或其他有副作用命令，必须得到用户明确确认后才执行。
8. 诊断时按“错误日志 -> 实际 INPUT -> 版本参数 -> 资源路径 -> 作业状态 -> 物理收敛”顺序排查，并给出最小修复。
9. 最终输出修改摘要、验证命令和结果、未验证风险以及下一步建议。

10. 后处理时探测实际文件名。ABACUS 版本可能输出 `band.txt` 而不是 `BANDS_1.dat`；不能把单一文件名当作成功标准。若能带数量不足，检查 `nbands` 和输出列数，明确说明需要重新计算而不是修改绘图脚本。
